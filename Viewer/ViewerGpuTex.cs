    ...
    public partial class Viewer
    {
        public partial class GpuTex
        {
                            ...
                            ImageRgba32 img)
            {
                ...
                m_nWidth    = img.nWidth; 
                m_nHeight   = img.nHeight;
            }

            public void ReplaceWith(ImageRgba32 img)
            {
                if (    (img.nHeight != m_nHeight) ||
                        (img.nWidth != m_nWidth))
                {
                    throw new InvalidDataException("Dimensions of replacement image must equal the original image's dimensions");
                }
                ...
                {
                    IntPtr ptr = handle.AddrOfPinnedObject();
                    _Refresh(oViewer.hThis, hThis, ptr);
                }
                finally
                {
                    handle.Free();
